...
#	K=3.36E-6
	...
	if myrank == 0:
		for i in range(1, size):
			Z = Particle[i-1][28]; R = Particle[i-1][29]; #Particle Current Position in Array
			...
			Particle[i-1][28]=Z; Particle[i-1][29]=R; #Particle Current Position in Array
...
#					print ("rank, pos = ", i, pos)
					...
					OrgZ = Particle[i-1][28]; OrgR = Particle[i-1][29]; #Cache Origin Revised
					CaZ = Particle[i-1][3]; CaR = Particle[i-1][4]
					MZ = CaZ//2; MR = CaR//2
					HZ = (CaZ * 3) // 4; LZ = CaZ // 4
					HR = (CaR * 3) // 4; LR = CaR // 4
...
#		Mob = float(Particle[myrank-1][3]); 
		...
		OrgZ = Particle[myrank-1][28]; OrgR = Particle[myrank-1][29]
...
#		print("DZ, DR, Ele shape = ", DZ.shape, DR.shape, Ele.shape)
		...
		posp = mdpybind.Position(z, r, vz, vr, 0, 0, OZ, OR)
		VV = VV.reshape(1,HCyc*CaZ*CaR)

		conp = mdpybind.Condition(Pcy, Pcyn, Step, CaZ, CaR, acc, Vis, gx, gy) #conditions
		posp.show()
		conp.show()
#		print("z, r, Pcy, Pcyn = ", posp.z, posp.r, conp.cur, conp.nex)
		for it in range(Maxiter):
#			print("before mdcal\n")
			posp = mdpybind.mdcal(posp, conp, VV)
#			posp.show()
			cyc += 1
			t += h
...
if myrank == 0:
	...
	trj = pd.DataFrame([[0, 0, 0, 0, 0, 0, 0]], columns = ['t', 'z', 'r', 'vz', 'vr', 'dvz', 'dvr'], dtype = np.float)
	...
	trj = pd.DataFrame([[0, CenterZ, CenterR-Nr//2, 0, 0, 0, 0]], columns = ['t', 'z', 'r', 'vz', 'vr', 'dvz', 'dvr'], dtype = np.float)
	print(trj)
